...
flip_matrix = function(X, dim_to_rev = NA){
  ...
  if(dim_to_rev == "R"){
    ...
    stop("dim_to_rev must not have row/column equivalency.")
  }
}
#TO-DO
```
...
#TO-DO
#We need to pick names without duplication
#nested 4 loops of 4 levels
```
...
It is 150 irises with four features: Sepal Length and Width, and Petal Length and Width. It also includes the specific species of each flower.
...
```{r}
dfDrop = subset(df, df$Species != "virginica")
```
...
mode = function(values){
  uniq = unique(values)
  set = tabulate(match(values, uniq))
  uniq[set == max(set)]
}
mode(q)
g = tabulate(q)
...
```{r}
threshold.model = function(sepalLen, species){
  y = c()
  lengths = length(sepalLen)
  numErrors = c()
  for (j in 1:100){
    threshold = dfDrop$Sepal.Length[j]
    for(i in 1:100){
      subject = sepalLen[j]
      if (subject <= threshold){
       y = append(y,"Setosa")
      }else{
        y = append(y,"versicolor")
      }
      numErrors[i] = sum(y != species)
      #check for true species Y
    }
  }
  y
}
...
dfDrop$Sepal.Length[4]
truth
  truth = cbind(dfDrop$Species)
  truth = (truth - 1)
  g = c()
  for (i in 1:100){
    if (y[i] == truth[i]){
...
?cbind.data.frame
?c
threshold.model(dfDrop$Sepal.Length, dfDrop$Species)
w = mean(dfDrop$Sepal.Length[1:100])
#TO-DO
```
```{r}
threshold.model <- function(sepal.length, true.species){
  n <- length(sepal.length)
  num.errors.by.parameter = matrix(NA, nrow = n, ncol = 2)
  for (i in 1:n){
    threshold <- sepal.length[i]
    numErrors = sum((sepal.length > threshold) != true.species)
    num.errors.by.parameter[i,] = c(threshold, numErrors)
  }
  bestRow = order(num.errors.by.parameter[,numErrors])[1]
  threshold = num.errors.by.parameter[bestRow, threshold]
  threshold
}
dfDrop$Species
threshold.model(dfDrop$Sepal.Length, dfDrop$Species)
...
In this picture, we have our 6 cases plotted out. Blue for 1, red for 0, and their position represents their feature values, x coords being feature 1, y coords feature 2.
...
threshold.model <- function(sepal.length, true.species){
  ...
  for (i in 1:n){
    threshold <- sepal.length[i]
    numErrors = sum((sepal.length > threshold) != species)
    num.errors.by.parameter[i] = c(threshold, numErrors)
  }
  bestRow = order(num.errors.by.parameter[,numErrors])[1]
  threshold = num.errors.by.parameter[bestRow, threshold]
...
```
```{r}
#basically pretend you're making some package for R
#' TO-DO: Provide a name for this function 
#'
#' TO-DO: Explain what this function does in a few sentences
#'
...
#' @param d           TO-DO: distance function(default euclid)
...
nn_algorithm_predict = function(Xinput, y_binary, Xtest, d = NULL){
  ...
  for(istar in 1:nrow(Xtest)){
    ...
    for (i in 1:nrow(Xinput)){
      dists[i]= d(Xinput[i, ], Xstar[istar, ]) #TO-DO
    }
  }
#  for(istar in 1:nrow(Xtest)){
#    y_hat[istar] = y_binary[which.min(rowSums(t(t(Xinput) - Xtest[istar, #]) ^ 2))]
  ...
  y_hat
  #lenX = Xinput.length
  #distMatrix = matrix(NA, nrow = lenX, ncol = 2)
  #TO-DO
}
```
