...
```{r}
storms[, average_diameter:=mean(c(tropicalstorm_force_diameter, hurricane_force_diameter), na.rm = TRUE), by = .I]
#TO-DO
```
...
setcolorder(storms, c("storm_max_wind_speed", "name"))
storms[order(-storm_max_wind_speed,year,month,day,hour)]
#storms[, index_storm_max_wind_speed := ]
#TO-DO
```
...
```{r}
#storms[, .(max(_wind_speed =max(wind))), by = year]
...
```{r}
storms[,.(max_category=max(category,na.rm = TRUE),max_wind=max(category,na.rm = TRUE), max_pressure=max(category,na.rm = TRUE),max_ts_diam=max(category,na.rm = TRUE),max_hu_diam=max(category,na.rm = TRUE)),by = name]
#TO-DO
#Substitute NA for neg inf
```
...
storms[(,.(num_storms = uniqueN(name)), by = c(year, category))]
#TO-DO
```
...
dist_miami = function(lats,longs){
  #go to internet find answer and paste on slack
}
n = length(lats)
...
max_wind_speed_Thus_far = max(wind), min_pressure = min(pressure), max(pressure), y = max_wind_speed), by = .(name,year)]
Xy[,category := factor(category, ordered = FALSE)]
Xy
